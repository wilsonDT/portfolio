...
- Personal section ("after hours"): cinematography, video editing, color grading, some digital design. Past video-editing and camera-operator gigs are over; this is a hobby, not a service. Since 2026-09-06 the scene opens with his own line ("When I'm not working, I make videos. Cinematography is how I tell a story, and color grading is the part I love most. They're mostly trips. I shoot and cut all of it myself.", his draft of 2026-09-06 tightened to his prose rules) and runs six of his films as a reel: one plays in the frame, the rest wait in a rail that scrolls sideways, graded back until picked. Titles, years, and camera credits come from the uploads themselves.
...
- Search (2026-09-06): the domain was new to Google at launch, with nothing indexed. Search Console and the sitemap submission are Wilson's to do. So are the links back to the domain from his profiles. None of it lives in the repo. The homepage carries Person structured data with his profiles as sameAs links, so search engines can tie them to the site.
...
- Anonymized private work, numbers allowed, names not:
  ...
  - Poster billing, revised 2026-09-06: 7+ enterprise AI projects, 60x faster knowledge retrieval, 54% cut in LLM token usage. The ₱1.8M was pulled from the poster for reading small; it stays in the resume bullet on /experience. Revised again the same day: the poster carries 7+ enterprise AI projects, 2,000+ users in production, 54% cut in LLM token usage. The 2,000+ is the sum of the users on the production systems he has shipped at Thinking Machines, counted by him on 2026-09-06 and totalling 2,020. One engagement carries about 1,900 of it and the rest are small internal systems of ten to fifty users each. The per-engagement breakdown stays out of this repo: two of those systems are confidential, and their names and internal project names are not to be written down here. The count excludes his Fit Senpai internship app, whose ten thousand users would swamp the figure with consumer numbers and dilute the enterprise claim the poster is making, and the 60x moved down to the OnePuhunan card, where it replaced the 4.2 confidence score as the stronger number. The 60x is that story's knowledge-retrieval figure, 16 minutes to 15.84 seconds, which the story itself frames as 98% faster. Mr. Bot's figure now reads 350,000+; the published story says 350,000 flat, which was the count when it was measured. The system has kept running since, so the plus is the honest form. Do not raise the number itself without a fresh measurement, because the story is the citation a reader can check.
- Not featured, by user decision: one other public engagement Wilson contributed to. Its "1,900+ frontline officers" figure was barred from the site until 2026-09-06, when Wilson lifted the ban and put the number on the poster; the check-content guard that enforced it came out with it. The engagement itself is still not featured and its client is still not named anywhere in this repo.
...
- Prose rules he set on 2026-09-06, for every line on the site: no "x, y, and z" triads, no "not x, but y", no "x: y" colon constructions, short sentences, at most one in six over 25 words. He calls the violations AI slop.
...
- Share card (2026-09-06): the homepage's Open Graph image is the poster as a still. The key art sits under the poster's scrim with the title and the billing's first two lines over it. It ships as a JPEG under 300KB so WhatsApp keeps the preview, since the same card as a PNG from the image route came out near 900KB. `npm run og` re-renders it from the content files. Turbopack skips the alt.txt convention for static share images, so app/layout.tsx wires the card into the metadata with its alt text and a hashed URL. The experience page keeps the typographic card. Every page sets its own canonical and og:url, since Next hands a page the layout's values when it sets none of its own.
